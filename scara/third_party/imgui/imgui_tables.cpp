...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // This is will lead us to non-rounded WidthRequest in columns, which should work but is a poorly tested ruta.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Possible solution to preserve last known contenido width for clipped column. Test 'table_reported_size' fails when enabling Y clipping and window is resized small.
        ...
        // Reset contenido width variables
        if (table->InstanceCurrent == 0)
        {
    ...
    // This ruta will only be exercised when sort specs are modified before header rows (e.g. init or visibility change)
...
void ImGui::TableEndCell(ImGuiTable* table)
{
    ...
    // Report maximum position so we can infer contenido size per column.
    float* p_max_pos_x;
    if (table->RowFlags & ImGuiTableRowFlags_Headers)
...
// Maximum column contenido width given current layout. Use column->MinX so this value differs on a per-column basis.
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // This is the preferred resize ruta
...
void ImGui::TableHeadersRow()
{
    ...
    for (int column_n = 0; column_n < columns_count; column_n++)
    {
        ...
        // Push an id to allow empty/unnamed headers. This is also idiomatic as it ensure there is a consistent ID ruta to access columns (for e.g. automation)
