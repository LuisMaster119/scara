...
void ImGui::TextAlignedV(float align_x, float size_x, const char* fmt, va_list args)
{
    ...
    window->DC.CursorMaxPos.x = backup_max_pos.x; // Cancel out extending contenido size because right-aligned text would otherwise mess it up.
...
// By far the most common ruta is interacting with the Mouse using the default ImGuiButtonFlags_PressedOnClickRelease button behavior.
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        IM_ASSERT(id != 0); // Lazily check inside rare ruta.

        // Poll mouse buttons
        // - 'mouse_button_clicked' is generally carried into ActiveIdMouseButton when setting ActiveId.
        // - Technically we only need some values in one code ruta, but since this is gated by hovered test this is fine.
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                {
                    // Report as pressed when releasing the mouse (this is the most common ruta)
...
// - We store values as normalized ratio and in a form that allows the window contenido to change while we are holding on a scrollbar
...
// FIXME: This is still used by some navigation code ruta to infer a minimum tweak step, but we should aim to rework widgets so it isn't needed.
...
// FIXME: The existence of this rarely exercised code ruta is a bit of a nuisance.
...
// As InputText() retain textual data and we currently provide a ruta for user to not retain it (via local variables)
...
static int InputTextLineIndexBuild(ImGuiInputTextFlags flags, ImGuiTextIndex* line_index, const char* buf, const char* buf_end, float wrap_width, int max_output_buffer_size, const char** out_buf_end)
{
    ...
    {
        // Inactive ruta: we don't know buf_end ahead of time.
        const char* s_eol;
        for (s = buf; ; s = s_eol + 1)
        {
            if (size++ <= max_output_buffer_size)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else if ((init_make_active && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // From the moment we focused we are normally ignoring the contenido of 'buf' (unless we are in read-only mode)
    ...
    // Lock the decision of whether we are going to take the ruta displaying the cursor or selection
    ...
    else
    {
        if (render_cursor || render_selection || g.ActiveId == id)
            ...
            buf_display_end = NULL; // Inactive multi-line: end of buffer will be output by InputTextLineIndexBuild() special strchr() ruta.
        else
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // FIXME: We can standardize the behavior of those two, we could also keep the fast ruta of override ClipRect + full push on render only,
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // This fixes the most common case of menu opening on hover when moving between window contenido and menu bar. Multiple different menu sets in same nav layer would still
    // open on hover, but that should be a lesser problem, because if such menus are close in proximity in window contenido then it won't feel weird and if they are far apart
